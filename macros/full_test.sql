{% macro full_test() %}
  ...
  {% for row in configs.rows %}
    ...
    {% else %}
      {% set high_watermark = none %}
    {% endif %}

    {% if high_watermark %}
      {% set src_filter_condition = src_load_timestamp ~ " > '" ~ high_watermark ~ "'" %}
    {% else %}
      {% set src_filter_condition = '1=1' %}
    {% endif %}


    {% if test_name == "row_count_match" %}
      {% do row_count_match(src_model, src_schema, src_db, test_id, test_config_id, trg_model, trg_schema, trg_db) %}
    ...
    {% elif test_name == "not_null_key" %}
      ...
      {% if keys | length > 0 %}
        {% do not_null_key(src_model, src_schema, src_db, test_id, test_config_id, keys) %}
      {% endif %}

    {% elif test_name == "unique_records" %}
      {% do unique_records(src_model, src_schema, src_db, test_id, test_config_id, src_hash_col) %}

    {% elif test_name == "hash_match" %}
      {% do hash_match(src_model, src_schema, src_db, src_pkey, src_hash_col, src_load_timestamp, test_id, test_config_id, trg_model, trg_schema, trg_db, trg_pkey, trg_hash_col, trg_load_timestamp) %}
    ...
    {% elif test_name == 'scd_columns_check' %}
      {% do scd_columns_check(src_model, test_id, test_config_id, src_schema) %}

    {% elif test_name == 'scd_unique_active_nkey' %}
      {% do scd_unique_active_nkey(src_model, test_id, test_config_id, src_nkey) %}

    {% elif test_name == 'scd_unique_surrogate_key' %}
      {% do scd_unique_surrogate_key(src_model, test_id, test_config_id, src_skey) %}

    {% elif test_name == 'scd_inactive_historical_records' %}
      {% do scd_inactive_historical_records(src_model, test_id, test_config_id) %}

    {% elif test_name == "scd_not_null_surrogate_key" %}
      {% do scd_not_null_surrogate_key(src_model, test_id, test_config_id, src_skey) %}

    {% elif test_name == 'scd_effective_before_expiry' %}
      {% do scd_effective_before_expiry(src_model, test_id, test_config_id) %}

    {% elif test_name == 'scd_initial_active_state' %}
      {% do scd_initial_active_state(src_model, test_id, test_config_id) %}

    {% elif test_name == "scd_initial_effective_date_set" %}
      {% do scd_initial_effective_date_set(src_model, test_id, test_config_id) %}

    {% elif test_name == "scd_active_null_expiry" %}
      {% do scd_active_null_expiry(src_model, test_id, test_config_id) %}

    {% elif test_name == 'scd_no_scd2_effective_date_gaps' %}
      {% do scd_no_scd2_effective_date_gaps(src_model, test_id, test_config_id, src_skey) %}

    {% elif test_name == "scd_sequential_versions" %}
      {% do scd_sequential_versions(src_model, test_id, test_config_id, src_skey) %}

    {% elif test_name == "scd_not_null_active_natural_key" %}
      {% do scd_not_null_active_natural_key(src_model, test_id, test_config_id, src_nkey) %}
    
    {% else %}
      {% do invok_macro(test_name, src_model, test_id, test_config_id) %}
    {% endif %}
  {% endfor %}

{% endmacro %}

{% macro invok_macro(macro_name, model_name, test_id, test_config_id) %}
